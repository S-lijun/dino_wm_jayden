...
def main():
    ...
    for backbone in backbones:
        ...
        if "maniskill" in args.task:
            ...
            ckpt_dir = Path(f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/carla/{backbone}")
            if backbone == "full_scratch":
                ckpt_dir = Path(f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/carla/vc1")
        ...
        wm.load_state_dict(torch.load(wm_ckpt_path, map_location=args.device)["model_state_dict"])
        fc = FailureClassifier(obs, wm, args).to(args.device)
        fc_ckpt_path = "/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/classifier/mlp_with_gp/maniskill3000classif/full_scratch/with-proprio/failure_classifier.pth"
        fc.load_state_dict(torch.load(fc_ckpt_path, map_location=args.device)["model_state_dict"])
...
if __name__ == "__main__":
    ...
    # bsub -n 24 -q general-interactive -Is -G compute-sibai -R 'rusage[mem=102GB]' -M 100GB -R 'gpuhost' -gpu "num=1:gmem=30G" -a 'docker(continuumio/anaconda3:2021.11)' -env "LSF_DOCKER_VOLUMES=/storage1/fs1/sibai/Active:/storage1/fs1/sibai/Active,LSF_DOCKER_SHM_SIZE=32g" /bin/bash
    # bsub -n 24 -q general-interactive -Is -G compute-sibai -R 'rusage[mem=102GB]' -M 100GB -R 'gpuhost' -gpu "num=1:gmem=30G" -a 'docker(maniskill/base)' -env "LSF_DOCKER_VOLUMES=/storage1/fs1/sibai/Active:/storage1/fs1/sibai/Active,LSF_DOCKER_SHM_SIZE=32g" -u y.yuxuan@wustl.edu /bin/bash
