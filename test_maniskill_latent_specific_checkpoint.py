...
class ManiskillEnvForTesting:
    ...
    def reset(self, state=None, seed=0):
...
class PIDController:
    def __init__(self, expert_ckpt_path="/storage1/fs1/sibai/Active/ihab/research_new/ManiSkill/examples/baselines/ppo/runs/UnitreeG1PlaceAppleInBowl-v1__ppo__8__1750699220/final_ckpt.pt"):
...
def simulate_dubins_with_hj(pid_controller, hj_evaluator, env, mode="switching", max_steps=200, 
    ...
    if save_video and frames:
        ...
        video_name = f"{info['success']}_dubins_hj_{mode}_{dynamics_suffix}_run{run_id}_{timestamp}.mp4"
...
def main():
    ...
    if backbone == "full_scratch":
        wm_ckpt_dir = f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/maniskill/vc1"
    else:
        wm_ckpt_dir = f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/maniskill/{backbone}"
    if finetune:
        backbone += "_ft"
    hj_ckpt_dir = f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/hj_ckpts/ddpg_hj_latent_mani_mlp_h/{backbone}/latest"
    if args.pid_only:
        video_save_path = f"/storage1/fs1/sibai/Active/ihab/research_new/dino_wm/close_loop/maniskill_{args.eps}/pid/"
    else:
        video_save_path = f"/storage1/fs1/sibai/Active/ihab/research_new/dino_wm/close_loop/maniskill_{args.eps}/{'dynamics' if args.only_dynamics else 'critic'}/{backbone}"
    ...
    for use_dynamics in dynamics_settings:
        ...
        for mode in modes:
            ...
            env = ManiskillEnvForTesting(device)
