...
class DubinsEnvForTesting:
    ...
    def reset(self, state=None):
        margin_r = self.env.hazard_size + 0.5

        if state is not None:
            while True:
                reset_out = self.env.reset(state=state)
                pos = self.env.state[:2]
                min_dist = min(np.linalg.norm(pos - h) for h in self.env.hazards)
                if min_dist > margin_r:
                    break
        else:
            while True:
...
def main():
    ...
    if backbone == "full_scratch":
        wm_ckpt_dir = f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/dubins/vc1"
    else:
        wm_ckpt_dir = f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/dubins/{backbone}"
    if finetune:
        backbone += "_ft"
    hj_ckpt_dir = f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/hj_ckpts/ddpg_hj_latent_dubins/{backbone}/epoch_200"
    # video_save_path = f"/storage1/fs1/sibai/Active/ihab/research_new/dino_wm/close_loop/dubins/'pid'/{backbone}"
    if args.pid_only:
        video_save_path = f"/storage1/fs1/sibai/Active/ihab/research_new/dino_wm/close_loop/dubins_{args.eps}/pid/"
    else:
        video_save_path = f"/storage1/fs1/sibai/Active/ihab/research_new/dino_wm/close_loop/dubins_{args.eps}/{'dynamics' if args.only_dynamics else 'critic'}/{backbone}"
