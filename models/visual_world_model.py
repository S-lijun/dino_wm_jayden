...
def _parse_image_hw(image_size):
...
class VWorldModel(nn.Module):
    def __init__(
        ...
        self.img_h, self.img_w = _parse_image_hw(image_size)
        decoder_scale = 16  # from vqvae
        self.n_patches_h = self.img_h // decoder_scale
        self.n_patches_w = self.img_w // decoder_scale

        if "dino" in self.encoder.name:
            enc_h = self.n_patches_h * encoder.patch_size
            enc_w = self.n_patches_w * encoder.patch_size
            self.encoder_image_size = (enc_h, enc_w)
            self.encoder_transform = transforms.Compose(
                [transforms.Resize((enc_h, enc_w))]
            )
        else:
