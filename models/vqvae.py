...
class VQVAE(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.n_patches_h is not None and self.n_patches_w is not None:
            h, w = self.n_patches_h, self.n_patches_w
            assert h * w == num_patches, f"patch grid {h}x{w} != {num_patches}"
        else:
            h = w = int(num_patches**0.5)
        input = rearrange(input, "b t (h w) e -> (b t) h w e", h=h, w=w)
