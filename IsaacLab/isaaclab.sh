...
extract_python_exe() {
    ...
    if [ ! -f "${python_exe}" ]; then
            ...
            if [ $(python -m pip list | grep -c 'isaacsim-rl') -gt 0 ]; then
                ...
                # use command -v: container /usr/bin/which breaks with "Illegal option --"
...
update_vscode_settings() {
    ...
    settings_template="${ISAACLAB_PATH}/.vscode/tools/settings.template.json"
    # check if the file exists before attempting to run it
    if [ ! -f "${settings_template}" ]; then
        echo "[WARNING] Missing '${settings_template}'. Skipping vscode settings setup."
        return 0
    fi
