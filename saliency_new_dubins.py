...
def load_data(task: str, root: str, seed=42, n_rollout=None, split_ratio=0.9):
    ...
    dset_path = Path("/storage1/fs1/sibai/Active/ihab/research_new/datasets_dino/dubins1800_continuous_cost")
    base_dset = Dataset(
        str(dset_path),
...
def main():
    ...
    # ---------- world‑model ----------
    ckpt_dir = Path(f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/dubins/{args.backbone}")
    ...
    hj_ckpt_dir = Path(f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/hj_ckpts/ddpg_hj_latent_dubins/{args.backbone}/epoch_200")
...
# means the patch is irrelevant for the value estimate.
    ...
    #python saliency_new_dubins.py --task cargoal --data_path "/storage1/fs1/sibai/Active/ihab/research_new/datasets_dino" --backbone r3m --occlusion_method blur --blur_sigma 11 --num_samples 20 --output_dir /storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/saliency/dubins --seed 3 --patch_size 20 --stride 10 --with_proprio
    #python saliency_new_dubins.py --task cargoal --data_path "/storage1/fs1/sibai/Active/ihab/research_new/datasets_dino" --backbone r3m --occlusion_method black --blur_sigma 11 --num_samples 20 --output_dir /storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/saliency/dubins --seed 3 --patch_size 20 --stride 10 --with_proprio
