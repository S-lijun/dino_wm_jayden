...
def visualize_saliency(image, saliency_map,
                       ...
                       save_path, overlay_only=True):
    """Save original, heat‑map, and overlay panels, or just overlay if overlay_only=True."""
    
    img_vis = to_uint8(unnormalize(image).permute(1, 2, 0).cpu().numpy())
    
    if overlay_only:
        # Save only overlay as JPG
        ...
        ax.imshow(img_vis, interpolation="bilinear", resample=True)
        ax.imshow(sal_norm, cmap=cmap, alpha=0.5, interpolation="bilinear", resample=True)
        ax.axis("off")
        plt.tight_layout()
...
def main():
    ...
    cls_path = Path(f"/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/classifier/mlp_with_gp/{args.task}")
    # cls_path= #to fix
...
# python saliency_new_cargoal.py --task cargoal --data_path "/storage1/fs1/sibai/Active/ihab/research_new/datasets_dino" --backbone dino_cls --occlusion_method white --blur_sigma 11 --num_samples 20 --output_dir /storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/saliency/cargoal_seed3 --seed 3 --patch_size 32 --stride 10 --with_proprio
