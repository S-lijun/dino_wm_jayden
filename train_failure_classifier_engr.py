...
def get_args_and_merge_config():
    ...
    parser.add_argument(
        ...
        default="/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2",
...
def main():
    ...
    backbones = [ "resnet","dino_cls","scratch", "vc1", "r3m", "dino","full_scratch"]
    # reverse the list
    ...
    for backbone in backbones:
        ...
        if "maniskill" in args.task:
            ckpt_dir = Path(f"/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2/maniskill/{backbone}")
            if backbone == "full_scratch":
                ckpt_dir = Path(f"/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2/maniskill/vc1")
        elif "carla" in args.task:
            ckpt_dir = Path(f"/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2/carla/{backbone}")
            if backbone == "full_scratch":
                ckpt_dir = Path(f"/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2/carla/vc1")
        elif "dubins" in args.task:
            ckpt_dir = Path(f"/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2/dubins/{backbone}")
            if backbone == "full_scratch":
                ckpt_dir = Path(f"/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2/dubins/vc1")
        elif "cargoal" in args.task:
            ckpt_dir = Path(f"/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2/cargoal/{backbone}")
            if backbone == "full_scratch":
                ckpt_dir = Path(f"/storage1/sibai/Active/ihab/research_new/checkpt_dino/outputs2/cargoal/vc1")
        else:
...
if __name__ == "__main__":
    ...
    # bsub -n 12 -q general-interactive -Is -G compute-sibai -R 'rusage[mem=102GB]' -M 100GB -R 'gpuhost' -gpu "num=1:gmem=30G" -a 'docker(continuumio/anaconda3:2021.11)' -env "LSF_DOCKER_VOLUMES=/storage1/sibai/Active:/storage1/sibai/Active,LSF_DOCKER_SHM_SIZE=32g" /bin/bash
    # bsub -n 12 -q general-interactive -Is -G compute-sibai -R 'rusage[mem=102GB]' -M 100GB -R 'gpuhost' -gpu "num=1:gmem=30G" -a 'docker(maniskill/base)' -env "LSF_DOCKER_VOLUMES=/storage1/sibai/Active:/storage1/sibai/Active,LSF_DOCKER_SHM_SIZE=32g" -u y.yuxuan@wustl.edu /bin/bash
