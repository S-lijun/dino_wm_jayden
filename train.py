...
class Trainer:
    ...
    # Immediately log how many DDP processes were spawned and which device each is on:
        ...
        if self.cfg.has_predictor and self.train_predictor:
            self._keys_to_save.append("action_encoder_optimizer")
    ...
    def _best_track_path(self):
        return Path(self.cfg.saved_folder) / "checkpoints" / "best_track.json"

    def _load_best_track(self):
        path = self._best_track_path()
        if path.exists():
            with open(path) as f:
                data = json.load(f)
            self.best_val_loss = float(data["best_val_loss"])
            self.best_epoch = int(data["best_epoch"])
            log.info(
                f"Loaded best track: epoch {self.best_epoch}, val_loss {self.best_val_loss:.4f}"
            )

    def _save_best_track(self):
    ...
    def maybe_update_best_ckpt(self, val_loss):
        if not self.accelerator.is_main_process:
            return
        if val_loss >= self.best_val_loss:
            return
        epoch_ckpt = Path(self.cfg.saved_folder) / "checkpoints" / f"model_{self.epoch}.pth"
        best_ckpt = Path(self.cfg.saved_folder) / "checkpoints" / "model_best.pth"
        if not epoch_ckpt.exists():
            log.warning(f"Cannot update model_best: missing {epoch_ckpt}")
            return
        shutil.copy2(epoch_ckpt, best_ckpt)
        self.best_val_loss = float(val_loss)
        self.best_epoch = int(self.epoch)
        self._save_best_track()
        log.info(
            f"New best val_loss={self.best_val_loss:.4f} at epoch {self.best_epoch} -> {best_ckpt}"
        )
    ...
    def init_models(self):
        ...
        if self.encoder.latent_ndim == 1:  # if feature is 1D
            ...
            img_h, img_w = int(self.cfg.img_h), int(self.cfg.img_w)
            n_ph = img_h // decoder_scale
            n_pw = img_w // decoder_scale
            ...
            if self.decoder is None:
                if self.cfg.env.decoder_path is not None:
                    ...
                    self.decoder = hydra.utils.instantiate(
                        ...
                        n_patches_h=img_h // 16,
                        n_patches_w=img_w // 16,
                    )
    ...
    def init_optimizers(self):
        if not hasattr(self, "encoder_optimizer") or self.encoder_optimizer is None:
        ...
        if self.cfg.has_predictor:
            if (
                not hasattr(self, "predictor_optimizer")
                or self.predictor_optimizer is None
            ):
            ...
            if (
                not hasattr(self, "action_encoder_optimizer")
                or self.action_encoder_optimizer is None
            ):
        ...
        if self.cfg.has_decoder:
            if (
                not hasattr(self, "decoder_optimizer")
                or self.decoder_optimizer is None
            ):
    ...
    def run(self):
        ...
        for epoch in range(init_epoch, init_epoch + self.total_epochs):
            ...
            if self.epoch % self.cfg.training.save_every_x_epoch == 0:
                ...
                if epoch_log is not None and "val_loss" in epoch_log:
                    self.maybe_update_best_ckpt(epoch_log["val_loss"])
